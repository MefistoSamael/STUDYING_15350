...
next_symbol:
            ...
            cmp al, ' '                  ; chek if current symbol is
            je end_found                 ; space or end of string
            cmp al, '$'                  ; if it is - starting to reverse word
            je end_found
            jmp next_symbol
 
end_found:                               ; found space or '$' - save the beginning and the end of the word
            mov dx, si                   
            dec si                       ; going one symbol back
            mov bx, di                   ; in order to know the word's last symbol
 
reverse_word:                            ; inverse the word
            cmp bx, si                   ; check if we done with out transformations
            jae done
            
            mov al, [bx]                 ; make a replacement
            mov ah, [si]
